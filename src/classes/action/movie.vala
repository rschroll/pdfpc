...
namespace pdfpc {
    ...
    public class Movie: ActionMapping {
        ...
        protected void establish_pipeline(string uri) {
            ...
            bin.add_many(tee);
            bin.add_pad(new GhostPad("sink", tee.get_static_pad("sink")));
            Gdk.Rectangle rect;
            int n = 0;
            uint* xid;
            while (true) {
                ...
                var xoverlay = sink as Video.Overlay;
            ...
            this.pipeline = ElementFactory.make("playbin", "playbin");
        ...
        public virtual void toggle_play() {
            State state;
            ClockTime time = Util.get_timestamp();
